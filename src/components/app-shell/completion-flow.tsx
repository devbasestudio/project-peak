  ...
  if (stage === "quiz" && question) {
    return (
      <div className="mx-auto max-w-3xl">
        <header className="mb-4 flex items-center justify-between"><p className="font-mono text-[10px] font-bold uppercase tracking-[.2em] text-charcoal/45">SUMMIT CHECK · QUIZ</p><span className="mono text-xs text-charcoal/38">{questionIndex + 1} / {questions.length}</span></header>
        <div className="h-1 overflow-hidden rounded-full bg-charcoal/8"><div className="h-full rounded-full bg-charcoal transition-[width]" style={{ width: `${((questionIndex + 1) / questions.length) * 100}%` }} /></div>
        <section className="mt-5 overflow-hidden rounded-3xl border border-charcoal/15 bg-white shadow-sm">
          <header className="relative overflow-hidden bg-charcoal p-6 text-white sm:p-8"><AscentMark className="absolute -right-10 top-0 h-36 w-56 text-sky/15" /><p className="relative font-mono text-[10px] font-bold uppercase tracking-[.2em] text-sky">QUESTION {String(questionIndex + 1).padStart(2, "0")}</p><h1 className="relative mt-4 font-display text-2xl font-black leading-tight tracking-[-.035em] sm:text-4xl" lang={mm ? "my" : "en"}>{mm ? question.questionMm : question.questionEn}</h1></header>
          <div className="space-y-3 p-5 sm:p-7">{question.options.map((option, index) => {
            ...
            return <button key={option.id} type="button" disabled={Boolean(feedback) || answering} onClick={() => answer(option.id)} className={`flex min-h-14 w-full items-center gap-4 rounded-2xl border px-4 py-3 text-left transition ${isCorrect ? "border-charcoal bg-sky" : isWrong ? "border-charcoal/30 bg-paper" : isSelected ? "border-charcoal" : "border-charcoal/15 bg-white hover:bg-paper"}`}><span className={`mono grid h-8 w-8 shrink-0 place-items-center rounded-full border border-charcoal/15 text-xs font-bold ${isCorrect ? "bg-charcoal text-white" : "bg-paper"}`}>{String.fromCharCode(65 + index)}</span><span className="flex-1 text-sm font-semibold leading-6" lang={mm ? "my" : "en"}>{mm ? option.textMm : option.textEn}</span>{isCorrect ? <Check size={18} /> : isWrong ? <X size={18} className="text-charcoal/45" /> : null}</button>;
          })}</div>
